...
class BedestenApiClient:
    ...
    def __init__(self, request_timeout: float = 60.0):
        ...
        # Simple in-memory caches (speeds up repeated queries within session)
        self._search_cache: dict[str, dict] = {}
        self._doc_cache: dict[str, BedestenDocumentMarkdown] = {}
    ...
    async def search_documents_phrase_only(self, phrase: str) -> dict:
        """Perform a search sending ONLY the phrase (all other filters removed). Returns raw JSON dict."""
        logger.info(f"BedestenApiClient: Phrase-only search started. Phrase='{phrase}'")
        try:
            # cache first
            cached = self._search_cache.get(phrase)
            if cached is not None:
                logger.info("BedestenApiClient: Phrase-only search cache hit")
                return cached
            payload = {
                "data": {
                    "phrase": phrase
            ...
            logger.info("BedestenApiClient: Phrase-only search completed")
            # save to cache
            self._search_cache[phrase] = result
            return result
        except httpx.RequestError as e:
            logger.error(f"BedestenApiClient: HTTP request error during phrase-only search: {e}")
            raise
        except Exception as e:
            logger.error(f"BedestenApiClient: Error processing phrase-only search response: {e}")
            raise
    ...
    async def get_document_as_markdown(self, document_id: str) -> BedestenDocumentMarkdown:
        ...
        try:
            ...
            if document_id in self._doc_cache:
                logger.info("BedestenApiClient: Document cache hit")
            ...
            if mime_type == "text/html":
                ...
                # Offload conversion to a thread to avoid blocking the event loop
                import asyncio as _asyncio
                markdown_content = await _asyncio.to_thread(self._convert_html_to_markdown, html_content)
            elif mime_type == "application/pdf":
                # Offload conversion to a thread to avoid blocking the event loop
                import asyncio as _asyncio
                markdown_content = await _asyncio.to_thread(self._convert_pdf_to_markdown, content_bytes)
            else:
                logger.warning(f"Unsupported mime type: {mime_type}")
                markdown_content = f"Unsupported content type: {mime_type}. Unable to convert to markdown."
            
            md_obj = BedestenDocumentMarkdown(
            ...
            # save to cache
            self._doc_cache[document_id] = md_obj
            return md_obj
